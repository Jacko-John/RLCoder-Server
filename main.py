...
def retrieve_codeblocks(
    ...
    if inference_type in ["bm25", "unixcoder", "unixcoder_with_rl"]:
        ...
        queries = [
            "\n".join(
                [x for x in example.left_context.split("\n") if x.strip() != ""][
                    -context_len:
                ]
            )
        ...
        if args.enable_repocoder and inference_type == "unixcoder_with_rl":
            ...
            generations = generator.generate(
                examples, retrieved_codeblocks, args.generator_max_generation_length
            )

            queries = [
                query + "\n" + prediction
                for query, prediction in zip(queries, generations)
            ]

        if inference_type == "bm25":
            return queries, candidate_codeblocks
        elif inference_type == "unixcoder":
            return queries, retriever.retrieve(
                ...
                if args.disable_stop_block:
                    ...
                    candidate_codeblocks = [
                        ...
                        + [
                            CodeBlock(
                                "",
                                "Don't need cross file context for completion",
                                "",
                                y.language,
                                "",
                            )
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        query_tokens_id = tokenize(
            self.queries[idx], self.tokenizer, self.max_query_length, True
        )
        candidate_tokens_id = [
            tokenize(str(x), self.tokenizer, self.max_candidate_length, False)
            for x in self.candidates[idx]
        ]
        return (
            torch.tensor(query_tokens_id, dtype=torch.long),
            torch.tensor(candidate_tokens_id, dtype=torch.long),
    ...
    if args.eval:
        ...
        for name, examples in all_eval_examples.items():
            ...
            for _ in range(args.forward_generation_times):
                _, retrieved_codeblocks = retrieve_codeblocks(
                    args, temp_examples, bm25, retriever, name
                )
                ...
                if args.enable_generation:
                    generations = generator.generate(
                        ...
                        args.generator_max_generation_length,
                    )
                    ...
                    if not temp_generations:
                        ...
                        temp_generations = [
                            temp_generations[i] + generations[i]
                            for i in range(len(generations))
                        ]
                    for i in range(len(temp_examples)):
                        temp_examples[i].left_context = (
                            examples[i].left_context + temp_generations[i]
                        )
            ...
            if args.enable_generation:
                ...
                with open(
                    f"{args.output_dir}/{name}/prediction.jsonl", "w", encoding="utf-8"
                ) as f_pred:
                    for example, temp_generation in zip(examples, temp_generations):
                        f_pred.write(
                            json.dumps(
                                {"task_id": example.task_id, "pred": temp_generation}
                            )
                            + "\n"
                        )

                if name == "cceval_python":
                    results = compute_metric_stmt(
                        f"{args.output_dir}/{name}",
                        "data/cceval/python/test.jsonl",
                        language="python",
                    ...
                    results = compute_metric_stmt(
                        f"{args.output_dir}/{name}",
                        "data/cceval/java/test.jsonl",
                        language="java",
                        ts_lib="utils/build/java-lang-parser.so",
                    )
                elif name == "github_eval":
                    targets, temp_generations = [
                        "".join(x.target_code.split()) for x in examples
                    ], ["".join(x.split()) for x in temp_generations]
                    results["em"] = round(
                        sum(
                            [
                                (
                                    1
                                    if x[: min(len(y), len(x))]
                                    == y[: min(len(y), len(x))]
                                    else 0
                                )
                                for x, y in zip(temp_generations, targets)
                            ]
                        )
                        / len(temp_generations)
                        * 100,
                        4,
                    )
                elif name == "codereval_python":
                    results = eval_codereval(
                        f"{args.output_dir}/{name}",
                        "data/codereval/python/CEPythonRaw.jsonl",
                        language="python",
                        do_codereval=args.do_codereval,
                    )
                elif name == "codereval_java":
                    results = eval_codereval(
                        f"{args.output_dir}/{name}",
                        "data/codereval/java/CEJavaRaw.jsonl",
                        language="java",
                        do_codereval=args.do_codereval,
                    )
                    ...
                    results = compute_metric_stmt(
                        f"{args.output_dir}/{name}",
                        "data/repoeval/line_level/test.jsonl",
                        language="python",
                        ts_lib="utils/build/python-lang-parser.so",
                    )
                    ...
                    results = compute_metric_stmt(
                        f"{args.output_dir}/{name}",
                        "data/repoeval/api_level/test.jsonl",
                        language="python",
                        ts_lib="utils/build/python-lang-parser.so",
                    )
            ...
            if "codereval" in name:
                codereval_table.add_row(
                    [
                        "raw",
                        name,
                        len(examples),
                        f"{np.mean(losses):.4f}",
                        f"{np.exp(np.mean(losses)):.4f}",
                        results["count"],
                        results["all"],
                        results["self"],
                        results["slib"],
        ...
        print(
            "data_per_epoch:{}, batch_size:{}, sample_number:{}, epoch:{}, inner_epoch:{}, lr:{}".format(
                args.data_per_epoch,
                args.batch_size,
                args.sample_number,
                args.epoch,
        ...
        scheduler = get_linear_schedule_with_warmup(
            optimizer,
            num_warmup_steps=args.data_per_epoch
            // args.batch_size
            * args.epoch
            * args.inner_epoch
            * 0.2,
            num_training_steps=args.data_per_epoch
            // args.batch_size
            * args.epoch
            * args.inner_epoch,
        )
        ...
        for name, examples in all_eval_examples.items():
            ...
            if "codereval" in name:
                evaluate_table[name].field_names = [
                    "Epoch",
                    "Method",
                    "Dataset",
                    "Total Samples",
        ...
        for name, examples in all_eval_examples.items():
            ...
            if args.enable_generation:
                ...
                with open(
                    f"{args.output_dir}/result_init/{name}/prediction.jsonl",
                    "w",
                    encoding="utf-8",
                ) as f_pred:
                    for example, temp_generation in zip(examples, temp_generations):
                ...
                if name == "cceval_python":
                    results = compute_metric_stmt(
                        f"{args.output_dir}/result_init/{name}",
                        "data/cceval/python/test.jsonl",
                        language="python",
                        ts_lib="utils/build/python-lang-parser.so",
                    )
                    ...
                    targets, generations = [
                        "".join(x.target_code.split()) for x in examples
                    ], ["".join(x.split()) for x in generations]
                    results["em"] = round(
                        sum(
                            [
                                (
                                ...
                                for x, y in zip(generations, targets)
                            ]
                        )
                        / len(generations)
                        * 100,
        ...
        for epoch in range(args.epoch):
            ...
            training_examples = construct_dataset(
                training_raw_data, 100 if args.debug else args.data_per_epoch
            )
            # training_examples = construct_dataset(training_raw_data, 100)
            queries, retrieved_codeblocks = retrieve_codeblocks(
                args,
                training_examples,
                bm25,
                retriever,
                "github_training_{}".format(epoch),
                True,
            )
            training_examples_dup = [
                x for x in training_examples for _ in range(args.sample_number)
            ]
            ...
            labels = (
                torch.tensor([x for x in losses])
                .view(-1, args.sample_number)
                .argmin(-1)
            )
            ...
            dataset = CustomDataset(
                args.retriever_query_context_length,
                args.retriever_candidate_context_length,
                retriever.tokenizer,
                queries,
                retrieved_codeblocks,
                labels.tolist(),
            ...
            for inner_epoch in range(args.inner_epoch):
                for batch in dataloader:
                    ...
                    doc_texts_embeddings = retriever(
                        doc_ids.view(-1, doc_ids.shape[-1])
                    ).view(source_ids.shape[0], args.sample_number, -1)
                    logits = (
                        torch.einsum(
                            "ab,acb->ac", queries_embeddings, doc_texts_embeddings
                        )
                        * 20
                    )
                ...
                if args.enable_sft:
                    ...
                    for name, examples in all_eval_examples.items():
                        ...
                        if args.enable_generation:
                            if (
                                os.path.exists(
                                    f"{args.output_dir}/result_{inner_epoch}/{name}"
                                )
                                is False
                            ):
                                os.makedirs(
                            ...
                            with open(
                                f"{args.output_dir}/result_{inner_epoch}/{name}/prediction.jsonl",
                                "w",
                                encoding="utf-8",
                            ) as f_pred:
                                for example, generation in zip(
                                    examples, temp_generations
                                ):
                                    f_pred.write(
                                        json.dumps(
                                            {
                    ...
                    retriever.model.module.save_pretrained(
                        f"{args.output_dir}/retriever_cpkt/result_{inner_epoch}"
                    )
                    retriever.tokenizer.save_pretrained(
                        f"{args.output_dir}/retriever_cpkt/result_{inner_epoch}"
                    )
            ...
            for name, examples in all_eval_examples.items():
                ...
                if args.enable_generation:
                    if (
                        os.path.exists(f"{args.output_dir}/result_{epoch}/{name}")
                        is False
                    ):
                        os.makedirs(
                            f"{args.output_dir}/result_{epoch}/{name}", exist_ok=True
                        )
                    with open(
                        f"{args.output_dir}/result_{epoch}/{name}/prediction.jsonl",
                        "w",
                        encoding="utf-8",
                    ) as f_pred:
                        for example, generation in zip(examples, temp_generations):
                            f_pred.write(
                                json.dumps(
                                    {"task_id": example.task_id, "pred": generation}
                                )
                                + "\n"
                            )
                    ...
                    if name == "cceval_python":
                        results = compute_metric_stmt(
                            f"{args.output_dir}/result_{epoch}/{name}",
                            "data/cceval/python/test.jsonl",
                            language="python",
                            ts_lib="utils/build/python-lang-parser.so",
                        )
            ...
            retriever.model.module.save_pretrained(
                f"{args.output_dir}/retriever_cpkt/result_{epoch}"
            )
            retriever.tokenizer.save_pretrained(
                f"{args.output_dir}/retriever_cpkt/result_{epoch}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--generator_batch_size_per_gpu",
        default=32,
        type=int,
        help="Generator batch size per GPU",
    )
    parser.add_argument(
        "--generator_max_crossfile_length",
        default=512,
        type=int,
        help="Maximum cross-file length for the generator",
    )
    parser.add_argument(
        "--generator_max_context_length",
        default=1024,
        type=int,
        help="Maximum context length for the generator",
    )
    parser.add_argument(
        "--generator_max_generation_length",
        default=64,
        type=int,
        help="Maximum generation length for the generator",
    )
    parser.add_argument(
        "--disable_generator", action="store_true", help="Disable the generator"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help="Retriever model path",
    )
    parser.add_argument(
        "--retriever_batch_size_per_gpu",
        default=64,
        type=int,
        help="Retriever batch size per GPU",
    )
    parser.add_argument(
        "--disable_retriever", action="store_true", help="Disable the retriever"
    )
    parser.add_argument(
        "--retriever_query_context_length",
        default=256,
        type=int,
        help="Retriever query context length",
    )
    parser.add_argument(
        "--retriever_candidate_context_length",
        default=512,
        type=int,
        help="Retriever candidate context length",
    )

    parser.add_argument(
        "--inference_type", default="baseline", type=str, help="Inference type"
    )
    parser.add_argument(
        "--output_dir", default="results/baseline", type=str, help="Output directory"
    )
    parser.add_argument("--eval", action="store_true", help="Perform evaluation")
    parser.add_argument(
        "--enable_tqdm", action="store_true", help="Enable progress bar"
    )
    parser.add_argument(
        "--enable_generation", action="store_true", help="Enable generation"
    )
    parser.add_argument(
        "--debug", action="store_true", help="Debug mode, use a small dataset"
    )

    parser.add_argument(
        "--num_workers", default=14, type=int, help="Number of CPU cores"
    )
    parser.add_argument(
        "--weighted_keywords",
        action="store_true",
        help="Weight keywords when calculating loss during training",
    )
    parser.add_argument(
        "--enable_fixed_block",
        action="store_true",
        help="Use fixed length blocks when building candidates",
    )
    parser.add_argument(
        "--enable_sft",
        action="store_true",
        help="Train using supervised learning methods",
    )
    parser.add_argument(
        "--disable_stop_block", action="store_true", help="Disable the stop block"
    )

    parser.add_argument(
        "--enable_repocoder",
        action="store_true",
        help="Use the repocoder method during generation",
    )
    parser.add_argument(
        "--rlcoder_model_path",
        default="microsoft/unixcoder-base",
        type=str,
        help="Stage 1 model for repocoder",
    )

    parser.add_argument(
        "--do_codereval",
        action="store_true",
        help="Execute codereval evaluation in docker",
    )
    parser.add_argument(
        "--enable_forward_generation",
        action="store_true",
        help="Use progressive generation methods during inference",
    )
    parser.add_argument(
        "--forward_generation_times",
        default=4,
        type=int,
        help="Number of times for progressive generation",
    )

    parser.add_argument(
        "--epoch", default=20, type=int, help="Number of training epochs"
    )
    parser.add_argument(
        "--inner_epoch", default=1, type=int, help="Number of inner training epochs"
    )
    parser.add_argument("--batch_size", default=16, type=int, help="Batch size")
    parser.add_argument(
        "--sample_number", default=10, type=int, help="Number of samples"
    )
    parser.add_argument(
        "--data_per_epoch", default=2000, type=int, help="Amount of data per epoch"
    )
    ...
    args.generator_batch_size = (
        args.generator_batch_size_per_gpu * torch.cuda.device_count()
    )
    args.retriever_batch_size = (
        args.retriever_batch_size_per_gpu * torch.cuda.device_count()
    )
